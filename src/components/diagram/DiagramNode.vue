...
<template>
  ...
  >
    ...
    >
      ...
      <!-- Node Tooltip only shows if node is hovered AND no children are handling the hover -->
      <NodeTooltip v-if="isActive && isHovered && node.tooltipDescription" :text="node.tooltipDescription" />
    </div>
  </div>
</template>
